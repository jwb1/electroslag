namespace gli
{
    ...
    (
        extent1d const& Coord1d,
#ifdef GLI_HAS_ASSERT
        extent1d const& Extent1d
#else
        extent1d const&
#endif
    ) const
	{
        GLI_ASSERT(glm::all(glm::lessThan(Coord1d, Extent1d)));
		return static_cast<size_t>(Coord1d.x);
	}

	inline storage_linear::size_type storage_linear::image_offset(extent2d const& Coord2d, extent2d const& Extent2d) const
	{
		GLI_ASSERT(glm::all(glm::lessThan(Coord2d, Extent2d)));
		return static_cast<size_t>(Coord2d.x + Coord2d.y * Extent2d.x);
	}

	inline storage_linear::size_type storage_linear::image_offset(extent3d const& Coord3d, extent3d const& Extent3d) const
	{
		GLI_ASSERT(glm::all(glm::lessThan(Coord3d, Extent3d)));
		return static_cast<storage_linear::size_type>(Coord3d.x + Coord3d.y * Extent3d.x + Coord3d.z * Extent3d.x * Extent3d.y);
	}
	...
	inline void storage_linear::copy(
		...
		extent_type const& BlockCountExtent)
	{
		...
		for(size_t BlockIndexZ = 0, BlockCountZ = BlockCountExtent.z; BlockIndexZ < BlockCountZ; ++BlockIndexZ)
		for(size_t BlockIndexY = 0, BlockCountY = BlockCountExtent.y; BlockIndexY < BlockCountY; ++BlockIndexY)
		{
			...
			memcpy(DataDst, DataSrc, this->block_size() * BlockCountExtent.x);
		}
	}
