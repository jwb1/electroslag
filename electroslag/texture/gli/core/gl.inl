...
namespace detail
{
	...
	inline gl::format gl::translate(gli::format ThisFormat, gli::swizzles const& ThisSwizzles) const
	{
		GLI_ASSERT(ThisFormat >= FORMAT_FIRST && ThisFormat <= FORMAT_LAST);

		gl::format_desc const& ThisFormatDesc = this->FormatDesc[ThisFormat - FORMAT_FIRST];

		gl::format ThisFormatGL;
		ThisFormatGL.Internal = ThisFormatDesc.Internal;
		ThisFormatGL.External = ThisFormatDesc.External;
		ThisFormatGL.Type = ThisFormatDesc.Type;
		ThisFormatGL.Swizzles = this->compute_swizzle(ThisFormatDesc, ThisSwizzles);
		return ThisFormatGL;
	}
	...
	inline gl::swizzles gl::compute_swizzle(format_desc const& ThisFormatDesc, gli::swizzles const& ThisSwizzles) const
	{
		if (!this->has_swizzle(this->Profile))
			return swizzles(gl::SWIZZLE_RED, gl::SWIZZLE_GREEN, gl::SWIZZLE_BLUE, gl::SWIZZLE_ALPHA);

		bool const IsExternalBGRA = ((ThisFormatDesc.Properties & detail::FORMAT_PROPERTY_BGRA_FORMAT_BIT) && !has_swizzle(this->Profile)) || (ThisFormatDesc.Properties & detail::FORMAT_PROPERTY_BGRA_TYPE_BIT);

		return detail::translate(IsExternalBGRA ? gli::swizzles(ThisSwizzles.b, ThisSwizzles.g, ThisSwizzles.r, ThisSwizzles.a) : ThisSwizzles);
	}
}//namespace gli
